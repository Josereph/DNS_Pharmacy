...
const CONTROLLER = (window.BASE_URL || '') + '/controllers/Productocontroller.php';
...
function renderizarTabla(productos) {
    ...
    tbody.innerHTML = productos.map(function (p, i) {
        var img = p.imagen_url
            ? '<img src="' + window.BASE_URL + '/' + p.imagen_url + '" class="tabla-img" alt="' + p.nombre + '">'
...
function abrirModalProducto(datos) {
    ...
    if (datos) {
        ...
        if (datos.imagen) {
            document.getElementById('img-preview-src').src = window.BASE_URL + '/' + datos.imagen;
